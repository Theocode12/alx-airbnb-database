...
def make_random_inserts(table_name, col_names, types, no_of_records):
  global SEQ
  SEQ = 0
  if (len(col_names) != len(types)):
    raise Exception("types and must match columns count")
  for typ in types:
    if typ not in ['number', 'string', 'uuid', 'name', 'email', 'date', 'foreign'] \
...
def make_random_value_from_type(typ, table_name):
  ...
  if typ.startswith("number") and ":" in typ:
    ...
    if (meta) and (len(meta) == 0):
      raise ValueError('Please put meta \nSyntax - number:length[:{int | string}]')
    length = int(meta[0])
    ...
    if len(meta) > 2:
      try:
        ...
        raise ValueError('Please indicate stop. If stop is Indicated Please make sure it is a number.  \nSyntax - number:length[:{int | string}[:start:stop]]')
  ...
  if typ == "uuid":
    ...
    if tbl_ids := F_STORE.get(tbl_cln_name) and (ids := F_STORE.get(tbl_cln_name).get('ids')):
  ...
  if typ == "name":
    return get_random_text(4, 10, False, False) + " " + get_random_text(4, 10, False, False)
  if typ == "email":
...
def get_random_text(min_chars_len, max_chars_len, use_punctuations=True, use_space=True):
    ...
    def get_next_char():
        ...
        if len(char_type_history) > 1 and char_type_history[-1] == char_type_history[-2]:
...
# make_random_inserts('tbl_name', ['a', 'b', 'c', 'd'], ['string', 'enum:NGN:USD:EU:GBP', 'number', 'string:30'], 5)
# make_random_inserts('tbl_name', ['a', 'b', 'c', 'd'], ['uuid', 'name', 'email', 'string:30:add_punct'], 5)
# make_random_inserts('tbl_name', ['a', 'b', 'c', 'd'], ['uuid', 'name', 'email', 'date'], 5)
print(make_random_inserts('user', ['user_id', 'first_name', 'last_name', 'email', 'password_hash', 'phone_number', 'role'],
                            ['uuid', 'string', 'string', 'email', 'string:35', 'number:11:string' , 'enum:guest:host:admin']
                        , 6000))

print(make_random_inserts('property', ['property_id', 'host_id', 'name', 'description', 'location', 'pricepernight'],
                                ['uuid', 'foreign:user', 'string:10', 'string:30:add_punct', 'string:50:add_punct', 'number:3:int:0:9']
                    , 8000))
              
print(make_random_inserts('booking', ['booking_id','property_id', 'user_id', 'start_date', 'end_date', 'status'],
                                     ['uuid', 'foreign:property', 'foreign:user', 'date', 'date', 'enum:pending:confirmed:canceled']
                                     , 7500))

print(make_random_inserts('payment', ['payment_id', 'booking_id','amount', 'payment_date', 'payment_method'],
                                     ['uuid', 'foreign:booking:strict', 'number:3', 'date', 'enum:credit_card:paypal:stripe']
                                     , 7000))                                 

print(make_random_inserts('review', ['review_id','property_id', 'user_id', 'rating', 'comment'],
                                     ['uuid', 'foreign:property', 'foreign:user', 'number:1:int:1:5', 'string:30']
                                     , 4500))

print(make_random_inserts('message', ['message_id', 'sender_id', 'recipient_id', 'message_body'],
